...
def test(ds, is_regression=False, verbose=False, phase="Validation"):
    ds.reset()
    hits = 0
    mean_loss = 0
    while not(ds.iter_done()):
        ...
        # print(o)
        hits += 0 if is_regression else batch_hits(o, y)
    ...
    if verbose:
        print(phase + ('' if is_regression else " Accuracy: " + str(accuracy)) + " Mean Loss " + str(mean_loss))
    return accuracy, mean_loss

def train(nn, hp, val_hist, train_hist, logger, is_regression=True):
    cur_epoch = 1
    cur_iter = 1
    for i in range(1, hp.epochs+1):
        ...
        while not(hp.ds_train.iter_done()):
            ...
            if cur_iter % hp.validate_every_no_of_batches == 0:
                ...
                train_accuracy = 0 if is.0 * (y_true - y_pred).mean()
...
hp = hyperparams(ConfigEnum.DIABETES)
...
nn = model(hp.input_size, hp.output_size, hp.hidden_shapes, sigmoid, sigmoid_grad, MSE, MSE_grad, is_regression=True, has_dropout=hp.has_dropout, dropout_perc=hp.dropout_perc)

val_hist = historian()
train_hist = historian()
logger = nnlogger(hp.output_log, ("Epoch", "Phase", "Iteration", "Accuracy", "Loss") )
train(nn, hp, val_hist, train_hist, logger, is_regression=True, )
test(hp.ds_test, is_regression=True, verbose=True, phase="Test")
